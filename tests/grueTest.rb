...
class GrueTest < MiniTest::Unit::TestCase
  ...
  def test_move
    ...
    assert_equal @world.grue.grueCurrentRoom.title,room.return_title(nextMove)
  end
  ...
  def test_move_to_user
    (@world.rooms.length).times do
      ...
      (@world.grue.pathFinder.path_weights[@world.grue.grueCurrentRoom.title][@world.user.currentRoom.title].distance).times do
